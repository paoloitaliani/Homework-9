...
```
## Warning in file(file, "rt"): cannot open file 'covidweeklygrowths.txt': No such
## file or directory
```
...
```
## Error in quantile.default(as.vector(daysdata), probs = 0.9): missing values and NaN's not allowed if 'na.rm' is FALSE
```
...
```
## Error in `[.data.frame`(cvdata, , c(1, 167:178)): undefined columns selected
```
...
```
## Warning in log(cvdata$y): NaNs produced
```
...
```
## Warning in cbind(1, xx): number of rows of result is not a multiple of vector
## length (arg 1)
```

```
## Warning in leaps.setup(x, y, wt = wt, nbest = nbest, nvmax = nvmax, force.in =
## force.in, : 174 linear dependencies found
```

```
## Warning in max((1:np)[!rightorder]): no non-missing arguments to max; returning -
## Inf
```

```
## Warning in leaps.setup(x, y, wt = wt, nbest = nbest, nvmax = nvmax, force.in =
## force.in, : nvmax reduced to -Inf
```

```
## Warning in leaps.setup(x, y, wt = wt, nbest = nbest, nvmax = nvmax, force.in =
## force.in, : NAs introduced by coercion to integer range
```
...
```
## Error in pls::mvr(full.model, ncomp = 12, data = cvdata[-fold[[i]], ], : Invalid number of components, ncomp
```
...
|:----------------|----------:|-----:|---:|---:|---------:|----:|
|prediction error |        NaN|   NaN| NaN| NaN|       NaN|  NaN|
...
|           |   |   |   |   |   |   |   |   |   |   |
|:----------|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
|regsubsets | NA| NA| NA| NA| NA| NA| NA| NA| NA| NA|
|LASSO      | NA| NA| NA| NA| NA| NA| NA| NA| NA| NA|
|PCR        | NA| NA| NA| NA| NA| NA| NA| NA| NA| NA|
|PLS        | NA| NA| NA| NA| NA| NA| NA| NA| NA| NA|
